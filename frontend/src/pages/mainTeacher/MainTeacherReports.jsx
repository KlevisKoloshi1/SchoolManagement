...
import { getClasses, getStudents, getPerformanceReport, exportPerformanceReportPdf } from '../../api/mainTeacher'
...
export default function MainTeacherReports() {
  ...
  const classOptions = [
    { value: '', label: t('reports.selectClassFirst') },
    ...
    <div className="space-y-8 animate-fade-in">
      <div className="text-center sm:text-left">
        ...
        <div className="text-text-secondary max-w-2xl">
          {t('reports.performanceReport')} — {t('mainTeacher.selectClass')} → {t('reports.selectStudent')}
        </div>
      </div>
      ...
      <Card title={t('reports.performanceReport')} className="max-w-6xl">
        <div className="flex flex-wrap gap-4 mb-6">
          ...
          />
          <Select
            label={t('reports.selectStudent')}
            value={studentId}
            onChange={(e) => setStudentId(e.target.value)}
            options={[{ value: '', label: t('reports.selectStudentFirst') }, ...studentOptions]}
        ...
        {!studentId ? (
          <div className="text-center py-12 text-text-secondary">
            {classId ? t('reports.selectStudentFirst') : t('reports.selectClassFirst')}
          </div>
        ) : (
